...
var _ = Describe("[test_id:49999999][serial][disruptive][scheduler][byres] numaresources workload placement considering specific resources requests", func() {
	...
	Context("with at least two nodes suitable", func() {
		...
		DescribeTable("[tier1][ressched] a guaranteed pod with one container should be placed and aligned on the node",
			...
			Entry("[tmscope:pod] with topology-manager-scope: pod, using CPU as deciding factor",
				nrtv1alpha1.SingleNUMANodePodLevel,
				// required resources for the test pod
				corev1.ResourceList{
					corev1.ResourceCPU:    resource.MustParse("16"),
...
func setupNodes(fxt *e2efixture.Fixture, nodesState desiredNodesState) ([]nrtv1alpha1.NodeResourceTopology, string) {
	...
	for nIdx, nodeName := range nrtCandidateNames.List() {
		...
		for zIdx, zone := range nrtInfo.Zones {
			padRes := getFreeResources(nodesState.RequiredResources, nodesState.DecidingResource, baseload)
...
func getFreeResources(RequiredResources corev1.ResourceList, resName corev1.ResourceName, baseload baseload.Load) corev1.ResourceList {
	freeResources := RequiredResources.DeepCopy()
	gapResources := RequiredResources.DeepCopy()
	fmt.Printf("In getFreeResources: gapResources: %v, freeResources: %v, RequiredResources:%v", gapResources, freeResources, RequiredResources)
	...
	// enough. "Less" can be any amount, so we ensure that gap is >=
	// the estimated baseload for that resource.
	fmt.Printf("Before SubCoreResources: gapResources: %v, baseload.CPU: %v, baseload.Memory:%v\n", gapResources, baseload.CPU, baseload.Memory)
	e2ereslist.SubCoreResources(gapResources, &baseload.CPU, &baseload.Memory)
	fmt.Printf("After SubCoreResources: gapResources: %v, baseload.CPU: %v, baseload.Memory:%v\n", gapResources, baseload.CPU, baseload.Memory)

	ExpectWithOffset(1, gapResources.Cpu().Value()).To(BeNumerically("<", RequiredResources.Cpu().Value()), "free CPU not less than required CPU")
	ExpectWithOffset(1, RequiredResources.Cpu().Value()-gapResources.Cpu().Value()).To(BeNumerically(">=", baseload.CPU.Value()), "Difference between required CPUs and free CPUs not less than baseload CPU")

	ExpectWithOffset(1, gapResources.Memory().Value()).To(BeNumerically("<", RequiredResources.Memory().Value()), "free Memory not less than required CPU")
	ExpectWithOffset(1, RequiredResources.Memory().Value()-gapResources.Memory().Value()).To(BeNumerically(">=", baseload.Memory.Value()), "Difference between required Memory and free Memory not less than baseload Memory")
	...
	case corev1.ResourceCPU:
		...
		// ExpectWithOffset(1, RequiredResources.Cpu().Value()-freeResources.Cpu().Value()).To(BeNumerically(">=", baseload.CPU.Value()), "Difference between required CPUs and free CPUs not less than baseload CPU")
